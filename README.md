...
### `main` 에서 태그로 — 조건의 정본은 백엔드다 (#193)

**`main` 머지는 공개가 아니다.** 실제로 내보내는 것은 태그이고 그 앞에 조건이 있다. 오래
이 자리에는 구조도만 있었고 **조건이 오지 않았다** — 그래서 `v0.1.0` 이 아무것도 만나지 않고
지나갔다.

**조건의 정본은 백엔드 레포다** — `docs/git-workflow.md` §8.7 의 `main → 공개` 칸과 그것이
거는 `docs/deployment.md` §3. **배포는 두 이미지가 함께 서는 일이라 한 곳에서만 말할 수 있다.**
계약(`docs/openapi.yaml`)이 이미 그 레포에 있는 것과 같은 이유이고, 대가도 같다 — 이 레포만
클론한 사람은 조건에 닿지 못한다.

여기서는 프론트가 알아야 하는 넷만 든다.

- **번호는 이 레포가 독립적으로 센다** (백엔드 이슈 #453). 백엔드가 태그를 붙였다는 이유로
  프론트 번호를 올리지 않고 그 반대도 아니다. 번호를 묶으면 **이미지를 둘로 나눈 이유의 절반**
  을 되돌린다 — 한쪽 오타에 다른 쪽까지 롤백된다
- **같은 문자열이 서로 다른 것을 가리킨다.** 프론트 `v0.1.0` 과 백엔드 `v0.1.0` 은 같은 것이
  아니다. **태그를 레포 없이 부르지 않고**, 되돌리는 지시는 태그가 아니라 **SHA 로** 한다
- **짝이 없는 태그는 배포 후보가 아니다.** `v0.1.0` 은 GHCR 게시까지 끝났지만 함께 검증된
  백엔드가 없다 — *게시됐다*는 사실이지 *나갈 수 있다*는 사실이 아니다. 이 이미지는 설정을
  담지 않고 API 주소를 컨테이너가 뜰 때 받으므로(ADR-0012) **혼자서는 아무것도 아니다**
- **조합은 백엔드 `docs/releases.md` 가 한 행씩 든다.** *어느 프론트와 어느 백엔드가 함께
  검증됐는가*를 드는 유일한 자리이며, 스테이징 검증 뒤 production 으로 넘기기 전에 적는다.
  **적지 않으면 아무도 모른다** — 그것이 번호를 갈라 세는 대가다

> **태그를 붙이는 것 자체는 막지 않는다.** `v0.1.0` 을 지우지도 않는다 — 검증된 이미지의
> 롤백 좌표이고, 번호를 맞추자고 지우면 그 좌표가 사라진다. 막는 대신 **그 태그가 무엇을
> 뜻하는지**를 좁혔다: 게시는 배포가 아니다.
