...
/*
 * **커버 자리는 이 파일에 없다** (#153). `#134` 가 여기 두었던 `coverFact` 는 *"커버가
 * 있다"* 는 사실만 적는 함수였고, 그 이유는 계약에 바이트를 주는 길이 없어서였다
 * (§13-77 의 `[결정 필요]`). §13-78 이 `readReviewImage` 를 열면서 그 사실 칸이
 * **그림을 그리는 자리**로 바뀌었고, 그 자리는 상태 넷과 사람의 손 하나를 갖는다 —
 * `reviewImage.ts` 가 그것을 든다.
 */
