  ...
  return (
    <div className={css.field}>
      ...
      >
        <option value="">선택 안 함</option>
        {/*
         * **같은 이름은 여기 두 줄로 서지 않는다** (#144). 접는 자리는 후보를 만드는 쪽
         * (`conditionSources`)이고 이 자리가 아니다 — 계약이 받는 것은 이름이므로 두 줄은
         * 정말로 같은 값이고, 고를 것이 둘인 것처럼 보이는 것 자체가 거짓 정보다.
         *
         * **Step 3 의 선언은 그대로 둘이다.** 중복을 화면이 막지 않는다 — 막으면 계약이
         * 허용하는 원고를 화면이 거절한다 (`flagRemovedEntirely` 가 같은 판단이다).
         *
         * **`key` 는 여전히 자리(index)다** (#132). 접는 쪽이 위에 있으므로 이제 이름으로도
         * 겹치지 않지만, 겹치지 않는다는 사실이 **다른 파일에** 있는 동안 이름을 키로 삼는
         * 것은 그 파일에 대한 의존이다. 이 목록은 정렬도 필터도 없고 이름이 바뀌면 통째로
         * 다시 만들어지므로, 자리와 항목이 어긋난 채로 재사용될 창이 없다.
         *
         * **`value` 는 이름 그대로다** — 계약이 받는 것이 이름이다 (`ConditionParams`).
         */}
