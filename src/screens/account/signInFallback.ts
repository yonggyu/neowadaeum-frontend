...
/**
 * **한 시도가 서버에 만드는 nonce 의 최대** (#227).
 *
 * 이 화면에는 nonce 를 받는 길이 둘이다 — One Tap 하나(`requestGoogleIdToken`), 탈출구 하나
 * (`mountGoogleSignInButton`). 둘 다 `prepareSignIn` 을 지나며 **각자 자기 값을 받고**, 한 번
 * 통과하면 끝나는 값이라 돌려쓰지 않는다 (§13-87).
 *
 * **그래서 둘을 하나로 합치지 않았다** (#227 의 결정). 합치면 이 수가 1 이 되지만, 탈출구는
 * 자기 nonce 를 **사람이 그것을 누르는 순간** 받는다 — 나눠 쓰면 One Tap 의 침묵(최소 10초)과
 * 사람이 망설이는 시간이 그대로 그 값의 노출 창이 되고, 그것이 `#212` 가 닫은 자리(만료된
 * nonce 를 든 버튼)를 다시 연다. 계약이 이 값을 *수명이 짧고 한 번만 통과한다* 고 말한다.
 * 그리고 `/auth/nonce` 의 한도는 계약에 **수가 적혀 있지 않으므로**(인증 경로 셋이 함께 쓰는
 * IP 기준 창이라는 사실만 있다 — S-8) 2 를 1 로 줄여 얻는 것이 지금은 값으로 환산되지 않는다.
 *
 * **이 수는 이 파일이 지키는 성질에서 나온다** — `attempt` 이 자리를 반드시 내리고(#212),
 * `reveal` 은 `offered` 까지만 가며(#218), 그 자리는 아직 GIS 를 부르지 않는다. 그래서 시도
 * 하나가 세우는 탈출구는 **최대 하나**다. 셋째 nonce 가 생기려면 사용자가 다시 눌러야 하고,
 * 그 순간은 이미 다음 시도다.
 *
 * **코드가 이 수를 세지는 않는다.** 세는 자리를 두면 그 자리가 또 하나의 상태가 되고 어긋날
 * 수 있다 — 대신 `googleIdToken.test.ts` 가 한 시도를 끝까지 돌려 이 수와 맞춘다.
 */
export const MAX_NONCES_PER_ATTEMPT = 2
 ...
 *    는 사람이 *"안 떴다"* 고 말한 뒤에야 GIS 를 다시 부른다. **이 성질은 이제 이 자리의
 *    순서에만 기대지 않는다** — `googleIdToken.ts` 의 `oneTapAlive` 가 같은 것을 값으로 든다 (#227)
 ...
 *    최대는 **여전히 둘**이다 (One Tap 하나 · 탈출구 하나) — 위의 `MAX_NONCES_PER_ATTEMPT` 가
 *    그 문장을 값으로 든다 (#227)
