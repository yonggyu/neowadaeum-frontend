 ...
 * 화면에 남는다.
 *
 * ## 이 문은 검수자 쪽과 감사가 같지 않다 — 그래서 부르는 시점도 다르다 (#215)
 *
 * **계약이 두 문에 대해 다르게 적었다.** 검수자 쪽(`readReviewImage`)은 오퍼레이션 설명이
 * *열람이 이미지마다 남는다* 고 못박고 기록이 실패하면 바이트가 나가지 않는다고까지 적는데,
 * **이 오퍼레이션의 설명에는 그 문장이 없다** — 소유권 · 서명 URL 금지 · 캐시 금지만 있다.
 * 정정본 §13-78/1 은 승인 전 경로 전체를 *감사 안에 머문다* 로 묶었으므로, 부르는 쪽은
 * **감사가 걸린 것으로 보고** 판단한다 (둘이 다르면 정정본이 이긴다).
 *
 * **그래도 이 자리는 열자마자 부른다.** 검수자 쪽이 *사람이 누를 때만* 부르는 이유(#86 ·
 * §13-78/4)가 여기서는 서지 않는다.
 *
 * 1. **부풀지 않는다.** 검수 큐는 판정하러 훑는 자리라 자동으로 부르면 **열어 본 적 없는
 *    원고가 큐 길이만큼** 기록에 쌓인다. 작성자는 자기 원고 하나를 열고, 그때 부르는 것은
 *    그 원고가 실제로 들고 있는 자리뿐이다
 * 2. **감사가 답하려는 물음의 대상이 아니다.** 그 기록은 *누가 남의 원고 이미지를 봤는가*
 *    를 답한다 (S-5) — 소유자가 자기 것을 보는 줄은 그 물음을 흐리지 않는다
 * 3. **누르게 만들면 `#154` 가 닫은 자리가 되돌아온다.** 그 이슈가 고친 것이 *올렸다는
 *    사실만 알고 무엇을 올렸는지는 모른다* 이고, 한 번 더 누르게 하면 화면은 다시 그 상태로
 *    열린다
 *
 * 부르는 자리는 `screens/authoring/ImageSlotField.tsx` 하나이며, 거기서 **같은 키를 두 번
 * 부르지 않고 실패한 키를 저절로 다시 부르지 않는** 이유도 이 문단이다.
 */
